...
- `adapter_type` text; built-ins include `process`, `http`, `claude_local`, `codex_local`, `gemini_local`, `jcode_local`, `opencode_local`, `pi_local`, `cursor`, `hermes_local`, `hermes_gateway`, and `openclaw_gateway`
...
Heartbeat dispatch must fail closed when an agent references an adapter type
that is not currently registered. It must not reinterpret unknown adapter
configuration as a generic process invocation.
